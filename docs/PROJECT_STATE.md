...
- 原始告警仍保留在案件状态中；敏感字段脱敏与 JSONL/Webhook 入口已共用接入管线。
- 已接入 `POST /api/intake/wazuh`：使用 `WAZUH_WEBHOOK_SECRET` 的 HMAC-SHA256 + 时间戳签名，支持 Wazuh 单条、数组与 `alerts` envelope；默认未配置密钥时返回 503。
- 已接入 `GET /api/raw-alerts` 与“原始告警”页面，支持来源、接入状态、externalAlertId、分页筛选；接口只返回元数据，不回显已脱敏原始 payload。
