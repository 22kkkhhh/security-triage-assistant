...
export function RawAlertList() {
  ...
  return <section className="ui-panel space-y-5 p-5">
    ...
    {data && data.rows.length > 0 ? <>
      <div className="overflow-x-auto border border-[var(--ui-border)]"><table className="min-w-full text-left text-sm"><thead className="bg-[var(--ui-surface-secondary)] text-xs text-[var(--ui-text-secondary)]"><tr><th className="px-3 py-2.5">接收时间</th><th className="px-3 py-2.5">来源</th><th className="px-3 py-2.5">外部告警 ID</th><th className="px-3 py-2.5">状态</th><th className="px-3 py-2.5">关联案件</th><th className="px-3 py-2.5">脱敏</th><th className="px-3 py-2.5 text-right">操作</th></tr></thead><tbody className="divide-y divide-[var(--ui-border-subtle)]">{data.rows.map((row) => <tr key={row.id}><td className="whitespace-nowrap px-3 py-2.5 text-[var(--ui-text-secondary)]">{new Date(row.receivedAt).toLocaleString("zh-CN", { hour12: false })}</td><td className="px-3 py-2.5 text-[var(--ui-text-primary)]">{row.sourceType}</td><td className="max-w-56 truncate px-3 py-2.5 font-mono text-xs text-[var(--ui-text-primary)]" title={row.externalAlertId ?? "—"}>{row.externalAlertId ?? "—"}</td><td className="px-3 py-2.5"><span className="ui-badge ui-badge-neutral">{statusLabel[row.ingestStatus] ?? row.ingestStatus}</span></td><td className="px-3 py-2.5 font-mono text-xs text-[var(--ui-text-secondary)]">{row.caseId ? row.caseId.slice(0, 10) : "—"}</td><td className="px-3 py-2.5 text-[var(--ui-success)]">v{row.redactionVersion.replace(/^v/, "")}</td><td className="px-3 py-2.5 text-right"><Link href={`/raw-alerts/${row.id}`} className="ui-button ui-button-ghost px-2.5 py-1.5 text-xs">查看详情</Link></td></tr>)}</tbody></table></div>
