...
export function InvestigationNextActions({
  ...
  const navigate = (view: "investigation" | "analysis" | "records", targetId: string) => {
    const params = new URLSearchParams(searchParams.toString());
    params.set("view", view);
    router.replace(`${pathname}?${params.toString()}`, { scroll: false });
    window.setTimeout(() => scrollToInvestigationSection(targetId), 0);
  };
  ...
  return (
    <section id="investigation-next-actions" data-testid="investigation-next-actions" className="rounded-lg border border-blue-200 bg-blue-50/60 px-4 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <button type="button" className="rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700 focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-600" onClick={() => navigate("investigation", INVESTIGATION_SECTION_IDS.investigation)}>开始调查</button>
      </div>
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        {pending.length > 0 ? pending.map((item) => (
          <button key={item.id} type="button" className="rounded-md border border-blue-200 bg-white p-3 text-left hover:border-blue-400 hover:shadow-sm" onClick={() => navigate("investigation", INVESTIGATION_SECTION_IDS.checklist)}>
