...
export function EvidencePanel({ evidences, pinnedEvidenceIds = [], onTogglePin }: { evidences: Evidence[]; pinnedEvidenceIds?: string[]; onTogglePin?: (id: string) => void }) {
  return <Panel title={`证据中心（${evidences.length} 条）`}>
    {evidences.length === 0 ? <p className="text-sm text-neutral-500">暂无证据。</p> : <ul className="space-y-2">{evidences.map((evidence) => {
      ...
      return <li key={evidence.evidenceId} className={`rounded border px-3 py-2 ${pinned ? "border-blue-300 bg-blue-50/40" : "border-neutral-100 bg-neutral-50"}`} data-testid="evidence-row">
        ...
        <div className="mt-1 text-sm font-medium text-neutral-900">{evidence.title}</div><p className="mt-0.5 text-xs leading-5 text-neutral-600">{formatDateTimesInDisplayText(evidence.summary)}</p>{evidence.analystNote ? <p className="mt-0.5 text-xs text-neutral-500">研判备注：{evidence.analystNote}</p> : null}
        <div className="mt-2 flex flex-wrap items-center gap-2">{onTogglePin ? <button type="button" className="rounded border border-blue-200 px-2 py-1 text-xs font-medium text-blue-700 hover:bg-blue-50" onClick={() => onTogglePin(evidence.evidenceId)} aria-pressed={pinned}>{pinned ? "取消关键证据" : "标记为关键证据"}</button> : null}{evidence.rawAlertId ? <Link href={`/raw-alerts/${evidence.rawAlertId}`} className="rounded border border-neutral-200 px-2 py-1 text-xs font-medium text-slate-700 hover:bg-white" data-testid="evidence-source-link">查看来源</Link> : <span className="text-xs text-neutral-400">来源不可用</span>}</div>
      </li>;
    })}</ul>}
  </Panel>;
}
